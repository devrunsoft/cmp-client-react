...
export default function ServicePriceDropDown(props: ServicePriceDropDownProps) {
  ...
  return (
    <div className={styles.dropDownContainer}>
      <div
        className={`${styles.dropDownHeader} ${isOpen && styles.focus}`}
        onClick={toggling}
      >
        <div>
          <p className={selectedOption ? styles.selectedOption : ""}>
            {selectedOption ? selectedOption.Name : `Select a ${props.select}`}
          </p>
        </div>
        <svg
          width="8"
