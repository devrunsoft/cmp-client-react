...
export async function getAllServiceAppointmentEmergencyApi(OperationalAddressId: number): Promise<Either<Error, ServiceAppointmentEntity[]>> {
    try {
        var h = header();
        const response = await fetch(`${Api_URL}/ServiceAppointmentEmergency/OperationalAddress/${OperationalAddressId}`, {
            method: 'GET',
            headers: h,
        });
