...
import { AddServiceAppointmentEmergencyCommand } from "common/domain/command/service_appointment_emergency/add_service_appointment_emergency_command";
...
import { ServiceAppointmentEmergencyEntity } from "common/domain/entity/service_appointment_emergency";
...
export async function addServiceAppointmentApi(command: AddServiceAppointmentEmergencyCommand): Promise<Either<Error, BaseResponse<ServiceAppointmentEmergencyEntity[]>>> {
    try {
        var h = header();
        const response = await fetch(`${Api_URL}/ServiceAppointmentEmergency`, {
            method: 'POST',
            headers: h,
