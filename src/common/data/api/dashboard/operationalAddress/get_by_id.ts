...
export async function getOperationalAddress(Id: number): Promise<Either<Error, OperationalAddressEntity>> {
    try {
        var h = header();
        const response = await fetch(`${Api_URL}/OperationalAddressDashboard/${Id}`, {
            method: 'GET',
            headers: h,
        });
