...
export async function getAllOperationalAddress(): Promise<Either<Error, OperationalAddressEntity[]>> {
    try {
        var h = header();
        const response = await fetch(`${Api_URL}/OperationalAddressDashboard/GetAll`, {
            method: 'GET',
            headers: h,
        });
