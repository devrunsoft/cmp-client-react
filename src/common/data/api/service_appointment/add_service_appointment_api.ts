...
export async function addServiceAppointmentApi(command: AddServiceAppointmentCommand): Promise<Either<Error, BaseResponse<object>>> {
    try {
        var h = header();
        const response = await fetch(`${Api_URL}/ServiceAppointment`, {
            method: 'POST',
            headers: h,
